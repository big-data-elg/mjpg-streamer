...
void *worker_thread(void *arg)
{
    ...
    if(bind(sd, (struct sockaddr*)&addr, sizeof(addr)) != 0)
        perror("bind");
    // -----------------------------------------------------------
    char *udpbuffer = malloc(MAX_UDP_LENGTH);

    while(!pglobal->stop) {
        // UDP receive ---------------------------------------------
        memset(udpbuffer, 0, sizeof(udpbuffer));
        bytes = recvfrom(sd, udpbuffer, MAX_UDP_LENGTH, 0, (struct sockaddr*)&addr, &addr_len);
        // ---------------------------------------------------------
        filesize = bytes;
        IPRINT("recevied data length:%d\n", filesize);
...
#endif
        memcpy(pglobal->in[plugin_number].buf, udpbuffer, filesize);
